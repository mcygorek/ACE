...
public:
  ...
  virtual std::string name()const{return std::string("MultiSite");}

  int get_N_sites()const{ return tpos.size();}
  double get_E(int k)const{ return E_g.get_E(k); }
  double get_omega(int k)const{ return E_g.get_E(k)/Constants::hbar_in_meV_ps; }
...
*/
    ...
    for(size_t i=0; i<sites_row.size(); i++){
      double d;
      if(!Reader::canReadDouble(sites_row[i], d)){
        if(i==0 && sites_row[i]=="center"){
          ...
          double dtau=Reader::readDouble(sites_row[1],"MultiSite_sites: dtau");
          size_t Nsites=Reader::readSizeT(sites_row[2],"MultiSite_sites: Nsites");
          if(Nsites<1){
    ...
    if(print_omega_g!=""){
      ...
      for(int i=0; i<E_g.N; i++){
        ofs<<E_g.get_E(i)/Constants::hbar_in_meV_ps<<" "<<E_g.get_g(i)<<std::endl;
      }
    }
  ...
  virtual ModePropagatorPtr getModePropagator(int k)const{
    ...
    for(int i=0; i<N_sites; i++){
      double phase=phase_sign(k)*tpos[i]*get_E(k)/Constants::hbar_in_meV_ps;
...
//std::cout<<"TEST: hbar*g(k):"<<Constants::hbar_in_meV_ps*get_g(k)<<std::endl; 
//std::cout<<"TEST: HB_Base:"<<std::endl<<HB_base<<std::endl;    
//std::cout<<"TEST: HE:"<<std::endl<<HB<<std::endl;    
    ...
    if(use_IP){
      TimedepMatrixPtr mp=new TimedepMatrix_InteractionPicture(
...
//    return ModePropagatorPtr(new ModePropagator(sysdim,get_bath_init(k),HB,get_env_ops()));
  ...
  ModePropagatorGenerator_MultiSite(Parameters &param){
    setup(param);
  }
};
