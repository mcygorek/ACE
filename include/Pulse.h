...
public:
  //area in units of pi; dE in meV; c and FWHM in ps
  double c;
  double FWHM;
  double area;
  double dE;
  ...
  virtual std::complex<double> f(double x)const{
    ...
    return area * exp(-0.5*y*y)/(sqrt(2.*M_PI)*s)
          * exp(std::complex<double>(0,-dE*x/Constants::hbar_in_meV_ps));
  }
  ...
  void setup(const std::vector<std::string> &sv){
    ...
    dE=0.;
    if(sv.size()>3)dE=Reader::readDouble(sv[3],"pulse_gauss: detuning");
  ...
  Pulse_Gauss(const std::vector<std::string> &sv): c(0.),FWHM(1.){
    setup(sv);
  }
  Pulse_Gauss(double c_, double FWHM_, double area_, double dE_=0)
...
public:
  ...
  void setup(const std::vector<std::string> &sv){
    ...
    if(sv.size()>3)dE=Reader::readDouble(sv[3],"pulse_rect: detuning");
  ...
  Pulse_Rect(double c_, double width_, double area_, double dE_=0)
...
public:
  ...
  virtual std::complex<double> f(double x)const{
    ...
    double dw=dE/Constants::hbar_in_meV_ps;
    double sc=1;
    if(t*t>1e-10){
      sc=sin(dw/2.*t)/(dw/2.*t);
    }
    return dw/2.*area*exp(std::complex<double>(0, -detuning*t/Constants::hbar_in_meV_ps))*sc;
  }

  Pulse_rect_freq(double c_, double area_, double dE_, double detuning_)
