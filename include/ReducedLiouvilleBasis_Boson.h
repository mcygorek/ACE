...
//std::cout<<"accepted as element "<<v.size()<<std::endl;
  ...
  ReducedLiouvilleBasis_Boson(const Eigen::MatrixXcd &initial, int nr_climb, double threshold=1e-12){
    setup_from_initial(initial, nr_climb, threshold);
  }
  ReducedLiouvilleBasis_Boson(const Eigen::MatrixXcd &U_)
    : ReducedLiouvilleBasis(U_){
  }
  ReducedLiouvilleBasis_Boson(){
    use_reduce=false;
  }
};
