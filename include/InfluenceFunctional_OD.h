...
//  IF_OD_Dictionary dict;
  ...
  void calculate_dict(double zero=1e-12){
  ...
  void reduce_to_dict(){
    if(a.size()<1)return;
    if(a[0].dim_i!=dict.beta.size()){
      std::cerr<<"InfluenceFunctional_OD::reduce_to_dict: a[0].dim_i!=dict.beta.size()!"<<std::endl;
      exit(1);
    }
    for(size_t n=0; n<a.size(); n++){
      MPS_Matrix m(dict.get_reduced_dim(), a[n].dim_d1, a[n].dim_d2);
      std::vector<std::vector<int> > rev=dict.get_reverse_beta();
      for(int i=0; i<m.dim_i; i++){
        if(rev[i].size()<1){ 
          std::cerr<<"reduce_to_dict: rev[i].size()<1!"<<std::endl;
          exit(1);
        }
        for(int d1=0; d1<m.dim_d1; d1++){
          for(int d2=0; d2<m.dim_d2; d2++){
            m(i, d1, d2)=a[n](rev[i][0], d1, d2);
          }
        }
      } 
      a[n].swap(m);
  ...
  void expand_from_diagonal(const InfluenceFunctional_D &other){
    if(other.a.size()<1){
      a.clear();
      dict.set_default(2);
      calculate_closures();
...
std::cout<<"expand_from_diagonal: "<<other.lgroups.get_Ngrps()<<" -> "<<dict.get_reduced_dim()<<std::endl;
  ...
  void calculate(ModePropagator &mprop, int n_max, double ta, double dt){
    ...
    int NL=N*N;
    int ML=M*M;
    const Eigen::MatrixXcd &bath_init=mprop.get_bath_init();

    //Environment operators
    env_ops.resize(n_max-1); 
    for(size_t n=0; n<n_max-1; n++){ 
...
//            env_ops[n][i+1](m1*M+m2)=mprop.env_ops[i](m1,m2);
...
//              a[n](i*NL+j, d1, d2)=mprop.A[j][i](d1,d2);
              a[n](i*NL+j, d1, d2)=mprop.A[i][j](d2,d1);
            }
          }
        }
      }
    ...
    for(int i=0; i<NL; i++){
      for(int j=0; j<NL; j++){
        for(int xi0=0; xi0<M; xi0++){
          for(int xi1=0; xi1<M; xi1++){
            for(int d2=0; d2<ML; d2++){
              ainit(i*NL+j, 0, d2) += a[0](i*NL+j, xi0*M+xi1, d2)*bath_init(xi0,xi1);
            }
          }
        }
      }
  ...
  /* Adding the effects of two influence functional.
     Check: Transition may come either from one or the other functional, 
            but not from both at the same time.
     Maybe: revise exponentials for single modes and replace by 
       exp(-i\lambda A)(1-i\lambda B) ?
  */
  void join_and_sweep(const InfluenceFunctional_OD &other, RankCompressor &compressor, double keep_weight=0., bool do_sweep=true){
    //Implement later: other.a.size()>a.size() can be handeled using closures
    if(a.size()!=other.a.size()){
      std::cerr<<"InfluenceFunctional_OD::join: a.size()!=other.size()!"<<std::endl; 
      exit(1);
    }
    if(a.size()<1)return;
...
//std::cout<<"DICT: "; dict.print_beta(); std::cout<<std::endl;
//std::cout<<"OTHER.DICT: "; other.dict.print_beta(); std::cout<<std::endl;
//std::cout<<"NEWDICT: "; newdict.print_beta(); std::cout<<std::endl;

    std::vector<std::vector<int> > rev=dict.get_reverse_beta();
    std::vector<std::vector<int> > rev2=other.dict.get_reverse_beta();
    std::vector<std::vector<int> > newrev=newdict.get_reverse_beta();

    for(int n=0; n<a.size(); n++){
      MPS_Matrix b(newdict.get_reduced_dim(), a[n].dim_d1*other.a[n].dim_d1, a[n].dim_d2*other.a[n].dim_d2);
      b.fill(0.);

      //no-flip terms
      for(int i=0; i<NL; i++){
        int i_ind_new=newdict.beta[i*NL+i];  if(i_ind_new<0)continue;
        int i_ind=dict.beta[i*NL+i];         if(i_ind<0)continue;
        int i_ind2=other.dict.beta[i*NL+i];  if(i_ind2<0)continue;
         
        if(i*NL+i!=newrev[i_ind_new][0])continue; //don't add twice

        for(int d1=0; d1<a[n].dim_d1; d1++){
          for(int d2=0; d2<a[n].dim_d2; d2++){
            for(int od1=0; od1<other.a[n].dim_d1; od1++){
              for(int od2=0; od2<other.a[n].dim_d2; od2++){
    b(i_ind_new, d1*other.a[n].dim_d1 + od1, d2*other.a[n].dim_d2 + od2)=
                  a[n](i_ind,d1,d2) * other.a[n](i_ind2,od1,od2);
              }
            }
          }
        }
      ...
      for(int i=0; i<NL; i++){
        for(int j=0; j<NL; j++){
          if(j==i)continue;
          int ij=i*NL+j;
          int i_ind_new=newdict.beta[ij];  if(i_ind_new<0)continue;
          int i_ind=dict.beta[ij];  
          int i_ind2=other.dict.beta[ij];  
          //diagonal indices
          int id_ind=dict.beta[i*NL+i];  
          int id_ind2=other.dict.beta[i*NL+i];
  
          if(ij!=newrev[i_ind_new][0])continue; //don't add twice

          if(id_ind>=0 && i_ind2>=0){
            for(int d1=0; d1<a[n].dim_d1; d1++){
              for(int d2=0; d2<a[n].dim_d2; d2++){
                for(int od1=0; od1<other.a[n].dim_d1; od1++){
                  for(int od2=0; od2<other.a[n].dim_d2; od2++){
    b(i_ind_new, d1*other.a[n].dim_d1 + od1, d2*other.a[n].dim_d2 + od2)+=
                    a[n](id_ind,d1,d2) * other.a[n](i_ind2,od1,od2);
                  }
                }
              }
            }
          }
          if(id_ind2>=0 && i_ind>=0){
            for(int d1=0; d1<a[n].dim_d1; d1++){
              for(int d2=0; d2<a[n].dim_d2; d2++){
                for(int od1=0; od1<other.a[n].dim_d1; od1++){
                  for(int od2=0; od2<other.a[n].dim_d2; od2++){
    b(i_ind_new, d1*other.a[n].dim_d1 + od1, d2*other.a[n].dim_d2 + od2)+=
                    a[n](i_ind,d1,d2) * other.a[n](id_ind2,od1,od2);
                  }
                }
              }
            }
      ...
      if(do_sweep && n>0){
        ...
        sweep_block_low_to_high(n-1, compressor, keep_weight);
      }
  ...
  void join(const InfluenceFunctional_OD &other){
    RankCompressor_SVD dummy(0.);
    join_and_sweep(other, dummy, 0., false);
  }
...
//std::cout<<"TEST3: n: "<<n<<" env_ops.size(): "<<env_ops.size()<<" other.env_ops.size(): "<<other.env_ops.size()<<std::endl;
        ...
        if(env_ops[n].size()<=1){
          ...
          for(size_t i=0; i<other.env_ops[2*n+1].size(); i++){
            env_ops[n][i]=VectorOuterProduct(thisid, other.env_ops[2*n+1][i]);
          }
        }else if(env_ops[n].size()==other.env_ops[2*n+1].size()){
          Eigen::VectorXcd thisid=env_ops[n][0];
          Eigen::VectorXcd otherid=other.env_ops[2*n+1][0];
          env_ops[n][0]=VectorOuterProduct(thisid, otherid);
          for(size_t i=1; i<env_ops[n].size(); i++){
            env_ops[n][i]=VectorOuterProduct(env_ops[n][i], otherid)
                         +VectorOuterProduct(thisid, other.env_ops[2*n+1][i]);
          }
        }else{
...
//std::cout<<"TEST4: n: "<<n<<" env_ops.size(): "<<env_ops.size()<<" other.env_ops.size(): "<<other.env_ops.size()<<std::endl;
      ...
      if(tgrid.use_rep && tgrid.rep_unit>0 && tgrid.rep_unit==n-1){
  ...
  void join_and_sweep(ModePropagator &mprop, double ta, double dt, 
         const Eigen::MatrixXcd &bath_init, RankCompressor &compressor, 
         double keep_weight=0., bool do_sweep=true){

    if(a.size()<1)return;
    ModePropagator mprop2=mprop;
    ...
    int ML=M*M;
    if(dict.beta.size() != NL*NL){
      std::cerr<<"InfluenceFunctional_OD::join: dict.beta.size() != NL*NL!"<<std::endl; 
      exit(1);
    }
    if(bath_init.rows()!=M ||bath_init.cols()!=M){
      std::cerr<<"join_and_sweep: bath_init.rows()!=M ||bath_init.cols()!=M!"<<std::endl;
      exit(1);
    }

    IF_OD_Dictionary newdict(dict); 
    IF_OD_Dictionary odict=mprop.get_dict();
    newdict.join(odict);
    ...
    if(a.size()==1){
      std::cerr<<"join_and_sweep: a.size()==1 not implemented yet!"<<std::endl;
      exit(1);
    }
    ///IMPORTANT: A <-> a have swapped indices xi!


    for(int n=0; n<a.size(); n++){
      mprop.update(ta+n*dt, dt/2);
      mprop2.update(ta+(n+0.5)*dt, dt/2);

      std::vector<std::vector<Eigen::MatrixXcd> > *A1=&mprop.A;
      std::vector<std::vector<Eigen::MatrixXcd> > *A2=&mprop2.A;
      int odim_d1=ML; 
      int odim_d2=ML;
      int odim_mid=ML;
      ...
      if(n==0){
        odim_d1=1;
        A1=new std::vector<std::vector<Eigen::MatrixXcd> > ( NL,
          std::vector<Eigen::MatrixXcd>(NL, Eigen::MatrixXcd::Zero(ML,1)));
        for(int i=0; i<NL; i++){
          for(int j=0; j<NL; j++){
            for(int chi=0; chi<ML; chi++){
              for(int xi0=0; xi0<M; xi0++){
                for(int xi1=0; xi1<M; xi1++){
                   (*A1)[i][j](chi,0 )+=mprop.A[i][j](chi,xi0*M+xi1) * bath_init(xi0, xi1);
//xi0*M+xi1) * bath_init(xi0, xi1);
                }
              }
            }
          }
        ...
        odim_d2=1;
        A2=new std::vector<std::vector<Eigen::MatrixXcd> > ( NL,
          std::vector<Eigen::MatrixXcd>(NL, Eigen::MatrixXcd::Zero(1,ML)));
        for(int i=0; i<NL; i++){
          for(int j=0; j<NL; j++){
            for(int chi=0; chi<ML; chi++){
              for(int xi0=0; xi0<M; xi0++){
                 (*A2)[i][j](0, chi)+=mprop.A[i][j](xi0*M+xi0,chi);
              }
            }
          }
        }
      ...
      MPS_Matrix b(newdict.get_reduced_dim(), a[n].dim_d1*odim_d1, a[n].dim_d2*odim_d2);
      b.fill(0.);
      ...
      for(int i=0; i<NL; i++){
        for(int j=0; j<NL; j++){
          for(int k=0; k<NL; k++){
            for(int l=0; l<NL; l++){
              int i_ind=newdict.beta[j*NL+k];  if(i_ind<0)continue;

              int i_ind_new=newdict.beta[i*NL+l];  if(i_ind_new<0)continue;
              if(i*NL+l!=newrev[i_ind_new][0])continue; //don't add twice

              for(int d1=0; d1<a[n].dim_d1; d1++){
                for(int d2=0; d2<a[n].dim_d2; d2++){
                  for(int od1=0; od1<odim_d1; od1++){
                    for(int odm=0; odm<odim_mid; odm++){
                      for(int od2=0; od2<odim_d2; od2++){
    b(i_ind_new, d1*odim_d1 + od1, d2*odim_d2 + od2)+=
 (*A2)[i][j](od2, odm) * a[n](i_ind,d1,d2) * (*A1)[k][l](odm,od1);
                      }
                    }
                  }
                }
...
//      std::cout<<"a["<<n<<"]: "<<a[n].dim_i<<" "<<a[n].dim_d1<<" "<<a[n].dim_d2<<std::endl;
  ...
  void join(ModePropagator &mprop, double ta, double dt, const Eigen::MatrixXcd &init){
    RankCompressor_SVD dummy(0.);
    join_and_sweep(mprop, ta, dt, init, dummy, 0., false);
  }
  ...
  void read_env_ops_binary(std::istream &ifs){
    ...
    for(int i=0; i<env_ops.size(); i++){
      ifs.read((char*)&itmp, sizeof(int));
      env_ops[i].clear();
      env_ops[i].resize(itmp); 
      for(int j=0; j<env_ops[i].size(); j++){
        ifs.read((char*)&itmp, sizeof(int));
...
//dict.beta.clear(); dict.beta.resize(16, -1); dict.beta[0]=0; dict.beta[5]=1; dict.beta[10]=2; dict.beta[15]=3; dict.N=2; dict.calculate_reduced_dim();
...
//    int n_max=tgrid.n_tot;
    ...
    if(n_max!=get_rank()){
      std::cerr<<"InfluenceFunctional_OD::add_modes: n_max!=get_rank()!"<<std::endl;
      exit(1);
    }

std::cout<<"add_modes: before: dict: "; dict.print_beta(); std::cout<<std::endl;
    for(int i=0; i<mpg.get_N_modes(); i++){
      std::cout<<"Lead mode "<<i<<"/"<<mpg.get_N_modes()<<std::endl;
...
//      Eigen::MatrixXcd bath_init=mpg.get_bath_init(i);
      IF2.calculate(mpp.ref(), n_max*2, 0, dt/2.);
...
/*
std::cout<<"After expand_ops (step "<<i<<"): init: "<<rep.init.size()<<" M: "<<rep.M.dim_d1<<" "<<rep.M.dim_d2<<" ident: "<<rep.env_ops[0].size()<<std::endl;
        std::cout<<"init: "<<rep.init<<std::endl;
        std::cout<<"ident: "<<rep.env_ops[0]<<std::endl;
*/
      }
...
//        }
  ...
  InfluenceFunctional_OD(const IF_TimeGrid &tgr_, DiagBB &diagBB, RankCompressor &compressor){
    initialize();
    tgrid=tgr_;
  ...
  InfluenceFunctional_OD(const IF_TimeGrid &tgr_, int N){
    initialize();
    tgrid=tgr_;
