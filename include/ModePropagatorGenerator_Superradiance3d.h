#ifndef MODE_PROPAGATOR_GENERATOR_SUPERRADIANCE_3D_DEFINED_H
#define MODE_PROPAGATOR_GENERATOR_SUPERRADIANCE_3D_DEFINED_H

#include "ModePropagatorGenerator.h"
#include "Operators_Boson.h"
#include "SpectralDensity.h"

class ModePropagatorGenerator_Superradiance3d: public ModePropagatorGenerator{
public:

  std::vector<std::pair<double, double> > E_g;
  int M_max;
  
  double distance_over_lambda;
  ...
  virtual void setup(Parameters &param){
    ...
    distance_over_lambda=param.get_as_double("Superradiance_distance_over_lambda",0.);
  ...
  virtual ModePropagatorPtr getModePropagator(int k)const{
    ...
    double exparg=(dist_over_lambda/(2.*M_PI))*cos(phi);
  
    std::complex<double> phasefac1=1.;
    std::complex<double> phasefac2=exp(std::complex<double>(0, exparg));
    ...
    return ModePropagatorPtr(new ModePropagator(get_N(),M_max,HB,0,get_env_ops()));
...
//    return Operators_Boson::equilibrium(M_max,x);
  ...
  ModePropagatorGenerator_Superradiance3d(){
  }
  ModePropagatorGenerator_Superradiance3d(Parameters &param){
    setup(param);
  }
};
