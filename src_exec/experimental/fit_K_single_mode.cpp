...
int main(int args, char** argv){
  ...
  //mixed (thermal) initial state: 
  //guess(2)/dt^2 ~ g^2 (1+n)  ;  guess(3)/dt^2 ~ g^2 n 
  //=> g^2 = (guess(2)-guess(3))/dt^2
  //=> n = guess(3)/dt^2 / g^2
  bool skip_thermal=false;
  double g0, ntherm;
  if(guess(2)<=guess(3)){
    skip_thermal=true;
    std::cout<<"a="<<guess(2)<<" <= b="<<guess(3)<<" => No single thermal mode possible!"<<std::endl;
  }else{
    g0=sqrt( (guess(2)-guess(3))/dt/dt );
    ntherm= guess(3)/dt/dt/g0/g0;
    std::cout<<"g0="<<g0<<" ntherm="<<ntherm<<std::endl;
  }
  ...
  if(prefix!=""){
    ...
    for(int i=0; i<=2; i++){
      if(i==0 && skip_thermal)continue;

      double sign=1;
      double g=g1;
      std::string initial_M="{|0><0|_"+int_to_string(M)+"}";
      if(i==0){
        double x=Boson_Equilibrium_invert(100,ntherm);
        Eigen::MatrixXcd Eq=Boson_Equilibrium(M, x, 1./kB_in_meV_by_K);
        initial_M=Matrix_as_parameter(Eq);
        
        std::cout<<"x="<<x<<" nres="<<(Operators_Boson::n(M)*Eq).trace().real()<<std::endl;
      }
      ...
      std::cout<<prefixparam<<std::endl;
      {
        ...
        ofs<<"add_single_mode_from_file "<<prefixmparam<<" "<<initial_M<<std::endl;
      ...
      if(param.get_as_bool("print_generateBCF")){
        ...
        ofs<<"te "<<dt*n_tot<<std::endl;
        ofs<<"dt "<<dt<<std::endl;
        ofs<<"initial {"<<g<<"*(bdagger_"<<M<<"+b_"<<M<<")*"<<initial_M<<"}"<<std::endl;
