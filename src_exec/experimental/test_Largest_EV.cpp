...
int main(int args, char **argv){
  ...
  int dim=param.get_as_size_t("dim", 1000);
  int iter=param.get_as_size_t("iter", 1000);
  double epsilon=param.get_as_double("epsilon", 1e-15);
  bool reortho=param.get_as_bool("reortho",false);
  int restart_m=param.get_as_size_t("restart_m", 100);
  int restart_k=param.get_as_size_t("restart_k", 3);
...
//  int imax=0; for(int i=1; i<dim; i++){if(std::abs(solve.eigenvalues()(i))>std::abs(solve.eigenvalues()(imax))){imax=i;}}
  int imax=0; for(int i=1; i<dim; i++){if(std::real(solve.eigenvalues()(i))>std::real(solve.eigenvalues()(imax))){imax=i;}}
  ...
  for(int i=0; i<dim; i++){srt[i]=std::make_pair(solve.eigenvalues()(i), i);}
  ...
  for(int i=0; i<10 && i<dim; i++){std::cout<<" "<<srt[i].first;}std::cout<<std::endl;
...
//  std::complex<double> Arnoldi=Largest_EV_Arnoldi(v, iter, epsilon, [&A](const Eigen::VectorXcd &x){return A*x;}, 2, reortho);
  std::complex<double> Arnoldi=Largest_EV_KrylovSchur(v, iter, restart_m, restart_k, epsilon, [&A](const Eigen::VectorXcd &x){return A*x;}, 2);
  ...
  std::cout<<"|(A-lambda*I)*ev|="<<(A*ArnoldiEV-Arnoldi*ArnoldiEV).norm()<<std::endl;
  ...
  if(print_convergence!=""){
    ...
    for(int iter2=3; iter2<iter; iter2++){
      ...
      v=Eigen::VectorXcd::Ones(dim);
      v.normalize();
//      std::complex<double> Arnoldi=Largest_EV_Arnoldi(v, iter2, epsilon, [&A](const Eigen::VectorXcd &x){return A*x;}, 1, reortho);
//      std::complex<double> Arnoldi=Largest_EV_Arnoldi_BLAS(v, iter2, epsilon, [&A](const Eigen::VectorXcd &x){return A*x;}, 1);
      std::complex<double> Arnoldi=Largest_EV_KrylovSchur(v, iter2, restart_m, restart_k, epsilon, [&A](const Eigen::VectorXcd &x){return A*x;}, 1);
      Eigen::VectorXcd ArnoldiEV=v;
      time_point time_Arnoldi2=now();
