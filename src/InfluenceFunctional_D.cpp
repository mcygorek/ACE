...
#include "MPS.hpp"
#include "IF_Line_MPS_low_to_high.hpp"

namespace ACE{

  void InfluenceFunctional_D::calculate_closures(){
  ...
  void InfluenceFunctional_D::calculate_diagBB(DiagBB &diagBB, RankCompressor &compressor, int n_extra){
    lgroups=Coupling_Groups_Liouville(diagBB.groups);
    int NL=diagBB.get_dim()*diagBB.get_dim();
    double dt=tgrid.dt;
    int n_tot=tgrid.n_tot+n_extra;
    int n_stop=tgrid.n_tot;
...
//      IF_Line_MPS_low_to_high line2(n_cut, dt, diagBB, n_tot-n);
      ...
      compressor.high_end_multiply_and_compress(*this, line, sqrt(NL));
...
//      print_dims(); 
  ...
  void InfluenceFunctional_D::read_binary(const std::string &filename){
    MPS::read_binary(filename);
    tgrid.set_default(MPS::get_rank()-1);
    calculate_closures();
  }

  void InfluenceFunctional_D::set_none(int n_max, int N){
    int NL=N*N;
    a.resize(n_max);
